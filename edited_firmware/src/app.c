...
int pixel_display(char message,int counter, int startRow);
...
void APP_USBDeviceEventHandler(USB_DEVICE_EVENT event, void * eventData, uintptr_t context)
{
    switch(event)
    {
        ...
        case USB_DEVICE_EVENT_DECONFIGURED:
            ...
            LATBbits.LATB7 = 0;     // turn on LED1 (B7 LED on PCB is LED 3)
            //BSP_LEDOn  (APP_USB_LED_2);
            LATBbits.LATB15 = 0;    // turn on LED2 (B15 LED on PCB is LED2)
            ...
            else if( appData.hidDataReceived )
            {
                ...
                switch(appData.receiveDataBuffer[0])
                {
                    case 0x80:
                        ...
                        // display "Hello!" on OLED if in this case
                        display_clear();
                        char message[62];
                        int counter = 0, startRow, ii;
                        char displayString[62];
                        
                        for (ii=0;ii<62;ii++) {
                            displayString[ii] = receiveDataBuffer[ii+2];
                        }
                        
                        startRow = receiveDataBuffer[1];    //receiveDataBuffer[1] is the row int from cpp file
                        //sprintf(message,"%d",startRow);
                        sprintf(message,"%s",displayString);
                        while (message[counter]) {
                            pixel_display(message[counter],counter,startRow);
                            counter++;
                        }
                        display_draw();
                    ...
                    case 0x81:
                        ...
                        if(appData.hidDataTransmitted)
                        {
                            ...
                            // read the accelerometer and reply the x data instead of the button state
                            short accels[3];
                            int kk;
                            if (_CP0_GET_COUNT()<200000) {
                                for (kk=0;kk>6;kk++) {
                                    appData.transmitDataBuffer[kk+1] = 0x00;
                                }
                            }
                            else {
                                ...
                                appData.transmitDataBuffer[5] = accels[2]>>8;       // high bit of z acceleration
                                appData.transmitDataBuffer[6] = accels[2]&0xFF;     // low bit of z acceleration
                            }
...
int pixel_display(char message,int counter, int startRow) {
        ...
        for (col=0; col<5; col++) {
            for (row=0; row<8; row++) {
                display_pixel_set(row+startRow,col+(counter*5),pixels[row][col]);
            }
        }
}
