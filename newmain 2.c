...
int pixel_maker(char messsage) { // used now only as a single character
// get one character/string (start with one character)
// find corresponding row in ASCII table
// loop through each row and column in the 8x5 array turning on and off pixels using display_pixel_set
// use display_draw() to send gddram to the screen
    display_init();
    
    char ASCII_var, ASCII_array[5];
    ASCII_var = message - 0x20;
    ASCII_array = ASCII[ASCII_var]; // gives a 1x5 array of hex numbers
    // bitwise AND each value in ASCII_array with 8x1 column of 1's with bit-shifting after each AND
    int ii, dummy[8][1], symbol[5], line;
    dummy = [1 1 1 1 1 1 1 1];
    for (ii=0; ii<5; ii++) {
        line = ASCII_array[ii] & dummy;
        symbol[ii] = line;
    }
    ...
    for (col=0; col<5; col++) {
        for (row=0; row<8; row++) {
            display_pixel_set(row,col,symbol[row][col]);
        }
    }
